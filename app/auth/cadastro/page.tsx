  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    try {
      const { error } = await supabase.auth.signUp({
        ...
        password,
        options: {
          emailRedirectTo: process.env.NEXT_PUBLIC_DEV_SUPABASE_REDIRECT_URL || `${window.location.origin}/listas`,
        },
      })
      ...
      <div className="w-full max-w-sm">
        <div className="flex flex-col gap-6">
          <div className="flex flex-col items-center gap-2 text-center">
            ...
            <p className="text-sm text-muted-foreground">Crie sua conta e comece a economizar</p>
          </div>
          <Card>
            <CardHeader>
            ...
            <CardContent>
              <form onSubmit={handleSignUp}>
                ...
                <div className="mt-4 text-center text-sm">
                  Já tem uma conta?{" "}
                  <Link href="/auth/login" className="underline underline-offset-4 text-emerald-700">
                    Entrar
                  </Link>
                </div>
              </form>
